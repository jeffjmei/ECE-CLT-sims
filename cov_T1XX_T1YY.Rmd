...
sim_cov <- map(1:100, ~ {
  ...
  T1YY <- map_dbl(sim, "T1YY")
  return(
    list(
      T1XX_T1YY = cov(T1XX, T1YY)
    )
  )
})
T1XX_T1YY <- map_dbl(sim_cov, "T1XX_T1YY") %>% conf_int()
```
...
T1XX_T1YY_theory <- hxhx_hyhy_theory +
  2 * hxhx_hyey_theory +
  hxhx_eyey_theory +
  2 * hxex_hyhy_theory +
  4 * hxex_hyey_theory +
...
expect_true(
    (T1XX_T1YY_theory >= T1XX_T1YY[1]) &
      (T1XX_T1YY_theory <= T1XX_T1YY[2])
  )
})
```
